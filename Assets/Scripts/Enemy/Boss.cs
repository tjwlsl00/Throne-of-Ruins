...
public class Boss : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        currentHP = maxHp;
        UpdateBossHealthUI();
        StartCoroutine(Phase1Behavior()); // Phase1부터 시작
    ...
    void UpdateBossHealthUI()
    {
        ...
        BossHealthText.text = $"{currentHP} / {maxHp} ({hpRatio:F0}%)"; // HP 텍스트 업데이트

        healthBarImage.fillAmount = (float)currentHP / maxHp; // HP 바 업데이트 
    }
